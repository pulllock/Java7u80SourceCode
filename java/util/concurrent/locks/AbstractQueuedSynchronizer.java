...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         * mode.
         * 用于实现条件队列的单向链表
         * 1. 在阻塞队列里面，nextWaiter用来判断结点是共享模式还是独占模式
         * 2. 在条件队列里面，用来表示后继结点，Condition Queue是单向链表，是一个并发不安全的
         */
        Node nextWaiter;
     ...
     * 同步状态
     * 作为锁的时候，代表当前锁的状态：0代表没有被占用，大于0代表有线程持有当前锁，锁可以重入。
     */
    ...
    private void unparkSuccessor(Node node) {
        ...
        /**
         * 后继结点为空或者状态是已取消
         * 后继结点可能取消了等待，此时后继结点的waitStatus == 1
         * 此时需要从队尾往前找，找到waitStatus小于等于0的所有节点中排在最前面的
         */
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws == Node.SIGNAL)
            ...
            return true;
        /**
         * 前驱结点的waitStatus大于0，表示前驱结点取消了排队。
         * 进入阻塞队列的线程会被挂起，唤醒操作是由前驱结点完成的。
         * 所以如果前驱结点已经取消，说明前驱结点不能再唤醒当前结点了，
         * 需要给当前结点再找一个能唤醒当前结点的前驱结点。
         */
        if (ws > 0) {
            /*
             ...
             */
            do {
                // 从后往前找，一直找到一个前驱结点的waitStatus <= 0 的
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
            // 新找到的前趋结点的next设置为node
            pred.next = node;
        } else {
            /*
             ...
             */
            /**
             * 如果走到这里，说明waitStatus不等于1和-1，就只可能是0，-2，-3
             * 而在这之前的步骤中并没有设置waitStatus的值的，所以waitStatus只可能
             * 是初始化的时候的值：0
             * 使用cas将当前结点的前驱结点的waitStatus设置为-1
             */
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                ...
                /**
                 * 前驱结点是头结点，且获取成功
                 * p == head 说明当前结点虽然进入到了阻塞队列，
                 * 但是是阻塞队列的第一个，当前结点就可以尝试抢一下锁。
                 *
                 * 当前head可能是刚刚初始化话的node，刚初始化的结点没有设置线程，
                 * 也即是说当前的head结点不属于任何一个线程，所以可以尝试抢一下锁。
                 */
                 ...
                 * 走到这里说明：
                 * 1. 当前结点不是队列头
                 * 2. 是队列头，但是tryAcquire没有成功
                 *
                 * shouldParkAfterFailedAcquire如果返回true，说明当前线程需要被挂起，
                 * 也能说明当前结点的前驱结点的waitStatus == -1
                 * 如果返回false，说明不需要被挂起
                 */
    ...
    public final void acquire(int arg) {
        ...
        if (!tryAcquire(arg) &&
            // tryAcquire没有成功，需要把当前线程挂起，放到阻塞队列中
     ...
     * 判断结点是否已经转移到阻塞队列中
     *
     * 结点进入条件队列中的时候，初始化结点的waitStatus = Node.CONDITION
     *
     * signal的时候，会将结点从条件队列转移到阻塞队列
     */
    final boolean isOnSyncQueue(Node node) {
        /**
         * 如果结点从条件队列转移到阻塞队列中，waitStatus会设置为0
         *
         * 如果waitStatus还是-2，表示结点还在条件队列中
         *
         * 如果结点的前驱为null，说明肯定没有在阻塞队列中
         */
        if (node.waitStatus == Node.CONDITION || node.prev == null)
            return false;

        /**
         * 如果结点有了后继几点，肯定是在阻塞队列中了
         */
        ...
        /**
         * 该方法从阻塞队列的队尾开始往前遍历，如果找到相等的节点，说明当前结点在阻塞队列中
         * 否则就是不在阻塞队列中
         */
     ...
     * 完全释放独占锁
     * 返回值是release之前的state值
     * 对于最简单的操作：
     *     先lock.lock()，state变为1
     *     然后condition.await()，state变为0，释放锁
     *     该方法返回1
     * 如果该方法失败，会将结点设置为取消状态，抛异常
     */
    final int fullyRelease(Node node) {
        ...
        try {
            ...
            /**
             * 使用当前的state值作为release的参数，将所有锁都释放
             * 最后state等于0
             */
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private void unlinkCancelledWaiters() {
            ...
            while (t != null) {
                ...
                /**
                 * 表示已被取消，需要删除
                 * 条件队列里面waitStatus只可能是-2或者是0
                 * 如果不是-2表示是被取消的结点，需要清除出去
                 */
        ...
        public final void await() throws InterruptedException {
             ...
             *
             * 上面结点入队列之后，fullyRelease方法会完全释放独占锁
             *
            ...
            /**
             * 有两种情况退出循环：
             * 1. 如果当前的结点已经转移到阻塞队列了，isOnSyncQueue返回true
             * 2. 如果不在阻塞队列中，就需要进行阻塞，然后检查checkInterruptWhileWaiting != 0表示线程中断，跳出循环
             *
